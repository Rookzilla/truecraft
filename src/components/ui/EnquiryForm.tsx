...
function validateContactFields(values: { email: string; phone: string }) {
  if (!isValidEmail(values.email.trim())) return 'Please enter a valid email address.'
  if (values.phone && !/^\d{1,11}$/.test(values.phone)) return 'Please enter a phone number using up to 11 digits only.'
  return ''
}

function TextInput({ field }: { field: TextField }) {
  const name = getFieldName(field.controlId)
  ...
  const control = (
    <Form.Control
      ...
      onInput={
        isPhone
          ? (event) => {
              event.currentTarget.value = normalisePhone(event.currentTarget.value)
            }
          : undefined
      }
      pattern={isPhone ? '[0-9]{0,11}' : undefined}
...
export function EnquiryForm({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const contactError = validateContactFields(values)
  ...
  return (
    <Form className={compact ? 'compact-enquiry-form' : undefined} noValidate onSubmit={handleSubmit}>
      {showHeading ? (
